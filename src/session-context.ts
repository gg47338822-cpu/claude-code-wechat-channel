...
export function loadSessionContext(snapshotFile: string, lastSessionFile: string, historyFile?: string): string | null {
  // Priority: last-session.md > history-snapshot.md > session-snapshot.md
  // history-snapshot.md is the 小本本 generated by UserPromptSubmit hook (30 recent exchanges from jsonl)
  const files = [lastSessionFile, historyFile, snapshotFile].filter(Boolean) as string[];
